...
func main() {
	...
	doMainLoop()
	/*
		imageStream := make(chan interface{})
		go readStreamingEndpoint(endpoint, imageStream)

		images := make(map[string]string)

		for {
			jsImageList, ok := <-imageStream
			if !ok {
				return
			}
			imageList, ok := jsImageList.([]interface{})
			if !ok {
				return
			}
			for _, image := range imageList {
				typedImage, ok := image.(map[string]interface{})
				if !ok {
					return
				}
				...
				// @todo
				//log.Printf("%v\n", typedImage)
				var status, widgetId interface{}
				if status, ok = typedImage["state"]; !ok {
					log.Printf("Image data has no obligatory state field. Data: %v\n", typedImage)
					return
				}

				if widgetId, ok = typedImage["id"]; !ok {
					log.Printf("Image data has no obligatory id field. Data: %v\n", typedImage)
					return
				}

				sstatus, ok := status.(string)
				if !ok {
					log.Printf("Unexpected status type %T", status)
					return
				}
				swidgetId, ok := widgetId.(string)
				if !ok {
					log.Printf("Unexpected id type %T", widgetId)
					return
				}

				//log.Printf("widgetId: %v, status: %v", widgetId.(string), status.(string))
				switch knownHash, knownImage := images[swidgetId]; sstatus {
				case "normal":
					hash, ok := typedImage["hash"]
					if !ok {
						log.Printf("Valid image without hash: %v", typedImage)
						return
					}
					shash, ok := hash.(string)
					if !ok {
						log.Printf("Unexpected hash type: %V", hash)
						return
					}
					// New image has been introduced
					if !knownImage || knownHash != shash {
						if len(knownHash) == 0 && len(shash) > 0 {
							origFilename, _ := typedImage["original_filename"]
							sorigFilename, _ := origFilename.(string)
							go downloadImage(swidgetId, sorigFilename)
						}
						images[swidgetId] = shash
					}
				case "deleted":
					if knownImage {
						delete(images, swidgetId)
					} else {
						log.Printf("Notified about deletion of widget %v that was never introduced", swidgetId)
					}
				}
			}
		}
